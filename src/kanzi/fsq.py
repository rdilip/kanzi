...
class FSQ(Module):
    def __init__(
        ...
        jitter_spread: float = 0.0 # add uniform noise 
    ):
        super().__init__()
        ...
        if dim_out is not None:
            self.project_out = nn.Linear(effective_codebook_dim, dim_out)
        else:
    ...
    def bound(self, z: Tensor, eps: float = 1e-3) -> Tensor:
        ...
        # add a little noise to z before tanh so we maintain the bound
        if self.training:
            z += torch.randn_like(z) * self.jitter_spread
